    ...
    module ListColumnHelpers
      ...
      def active_scaffold_column_checkbox(column, record)
        ...
        if column.inplace_edit and record.authorized_for?(:action => :update, :column => column.name)
          ...
          script = remote_function(:method => 'POST', :url => {:controller => params_for[:controller], :action => "update_column", :column => column.name, :id => escape_id(record.id).to_s, :value => !column_value, :eid => params[:eid]})
